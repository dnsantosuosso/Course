...
from assignments.a1.a1.grouper import slice_list, windows
from assignments.a1.a1.survey import Question, Answer, Survey, YesNoQuestion
...
class TestCourse:
    ...
    def test_enroll_students1(self, course_with_students, students) -> None:
...
def test_get_answer() -> None:
    m = Student(5, 'Diego')
    q1 = YesNoQuestion(5, 'Question Text 1')
    a = Answer(True)
    m.set_answer(q1, a)
    q2 = Question(6, 'Question Text 2')
    assert m.get_answer(q2) is None


# CLASS COURSE: DIEGO -> Inside sample test text

# GROUPER.PY

def test_slice_list1() -> None:
    assert slice_list([1, 2, 3, 4, 5], 5) == [[1, 2, 3, 4, 5]]


def test_windows1() -> None:
    assert windows([1, 2], 2) == [[1, 2]]


if __name__ == '__main__':
    pytest.main(['example_tests.py','-k', 'TestLonelyMemberCriterion'])
